...
export default function ServicesPreviewClient({ initialCategories }: ServicesPreviewClientProps) {
  ...
  useEffect(() => {
    if (activeSection) {
      const tabEl = document.getElementById(`tab-mobile-${activeSection}`)
      if (tabEl) {
  ...
  return (
    <>
      ...
      {/* HERO */}
      <section style={{ padding: '6rem 2rem 2.25rem', textAlign: 'center', background: 'radial-gradient(ellipse at 50% 0%, rgba(0,210,255,0.06) 0%, transparent 60%)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.7rem', color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '1.5rem', fontFamily: 'var(--font-jetbrains)' }}>
            ...
            <span>/</span>
            <span>Services &amp; Pricing Preview</span>
          </div>
          <h1 className="font-syne" style={{ color: 'var(--text-primary)', fontWeight: 800, fontSize: 'clamp(2.5rem, 5vw, 4rem)', lineHeight: 1.05, letterSpacing: '-0.03em', marginBottom: '1rem' }}>
            Interactive Preview<br /><span style={{ color: '#00D2FF' }}>Spotlight Cards &amp; Sticky Menu</span>
          </h1>
              ...
              filteredCategories.map(cat => {
                ...
                return (
                  <section
                    key={cat.slug}
                    id={`s-${cat.slug}`}
                    style={{ marginBottom: isExpanded ? '3.5rem' : '1.5rem', scrollMarginTop: '130px' }}
                  >
                    {/* Section Header */}
                    <motion.div 
                      whileHover="hover"
